#ifndef ACTIONENGINE_UTIL_STATUS_MACROS_H_
#define ACTIONENGINE_UTIL_STATUS_MACROS_H_
...
//   }
//
// The macro ends with a `act::util::StatusBuilder` which allows the returned
...
//   }
//
// `act::util::StatusBuilder` supports adapting the builder chain using a
...
// interfaces.  See `act::util::TaskReturn` and `rpc::RpcSetStatus` for
// details.
//
//   void Read(StringPiece name, act::util::Task* task) {
//     int64 id;
...
// to avoid confusion between a `act::util::StatusBuilder` and a
...
#define RETURN_IF_ERROR(expr)                                           \
  ...
  if (act::util::status_macro_internal::StatusAdaptorForMacros          \
          status_macro_internal_adaptor = {(expr), ACTIONENGINE_LOC}) { \
  } else /* NOLINT */                                                   \
...
// only occurred in the success case.
//
// Interface:
//
...
// well as a `act::util::StatusBuilder` object populated with the error and
...
#endif  // ACTIONENGINE_UTIL_STATUS_MACROS_H_