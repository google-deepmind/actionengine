#ifndef EGLT_UTIL_STATUS_MACROS_H_
#define EGLT_UTIL_STATUS_MACROS_H_
...
//   }
//
// The macro ends with a `eglt::util::StatusBuilder` which allows the returned
...
//   }
//
// `eglt::util::StatusBuilder` supports adapting the builder chain using a
...
// interfaces.  See `eglt::util::TaskReturn` and `rpc::RpcSetStatus` for
// details.
//
//   void Read(StringPiece name, eglt::util::Task* task) {
//     int64 id;
...
// to avoid confusion between a `eglt::util::StatusBuilder` and a
...
#define RETURN_IF_ERROR(expr)                                        \
  ...
  if (eglt::util::status_macro_internal::StatusAdaptorForMacros       \
          status_macro_internal_adaptor = {(expr), EGLT_LOC}) { \
  } else /* NOLINT */                                                \
...
// well as a `eglt::util::StatusBuilder` object populated with the error and
...
#define STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_2_(lhs, rexpr)                  \
  STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_(                                     \
      ...
      return eglt::util::StatusBuilder(                                     \
...
#endif  // EGLT_UTIL_STATUS_MACROS_H_