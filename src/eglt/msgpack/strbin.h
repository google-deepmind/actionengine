...
#ifndef EGLT_MSGPACK_STRBIN_H
#define EGLT_MSGPACK_STRBIN_H

#include "eglt/msgpack/core_helpers.h"

namespace eglt::msgpack {
inline absl::StatusOr<uint32_t> GetStrOrBinExtent(
    const LookupPointer& data, std::string_view type_for_error) {
...
inline absl::Status EgltMsgpackSerialize(const std::string& value,
...
inline absl::StatusOr<uint32_t> EgltMsgpackDeserializeBin(
...
inline absl::Status EgltMsgpackSerialize(const std::vector<uint8_t>& value,
                                         const InsertInfo& insert) {
  SerializedBytesVector result, encoded_size;
