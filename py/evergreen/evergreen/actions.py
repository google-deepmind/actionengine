...
class ActionRegistry(actions_pybind11.ActionRegistry):
  ...
  def make_action_message(
    ...
    Returns:
      The action message.
    """
    return super().make_action_message(name,
                                       action_id)  # pytype: disable=attribute-error
  ...
  def make_action(
      ...
      *,
      ...
      stream: "EvergreenStream | None" = None,
...
class Action(actions_pybind11.Action):
  ...
  def get_registry(self) -> ActionRegistry:
    ...
    return utils.wrap_pybind_object(ActionRegistry,
                                    super().get_registry())  # pytype: disable=attribute-error

  # pytype: disable=name-error
  # pylint: disable-next=[useless-parent-delegation]
  def get_stream(self) -> "EvergreenStream | None":
    # pytype: enable=name-error
  ...
  def get_input(self, name: str, bind_stream: bool | None = None) -> AsyncNode:
    ...
    """
    return utils.wrap_pybind_object(
        AsyncNode, super().get_input(name, bind_stream)
  ...
  def get_output(self, name: str, bind_stream: bool | None = None) -> AsyncNode:
    ...
    """
    return utils.wrap_pybind_object(
        AsyncNode, super().get_output(name, bind_stream)
        # pytype: disable=attribute-error
    )
  ...
  def run(self) -> Awaitable[None]:
    ...
    background_task = utils.schedule_global_task(
        asyncio.to_thread(super().run))  # pytype: disable=attribute-error
