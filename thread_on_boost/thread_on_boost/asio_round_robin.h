...
class round_robin final : public algo::algorithm {
 private:
  ...
  struct service : public boost::asio::io_context::service {
    ...
    explicit service(boost::asio::io_context& io_ctx)
        : boost::asio::io_context::service(io_ctx),
          work_{boost::asio::make_work_guard(io_ctx)} {}
    ...
    void shutdown() override { work_.reset(); }
  };
  //]

  //[asio_rr_ctor
  explicit round_robin(const std::shared_ptr<boost::asio::io_context>& io_ctx)
